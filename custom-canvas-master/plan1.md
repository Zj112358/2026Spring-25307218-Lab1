...
- **任务**：
  - [ ] 将 `deviceTypes` 扩展为 `["phone", "tablet", "2in1"]`
  - [ ] 验证应用在三种设备上可安装启动
...
- **任务**：
  - [ ] 新增 `ScreenInfo.ets` 工具类，通过 `display.getDefaultDisplaySync()` 获取屏幕宽高（vp）
  - [ ] Canvas 组件改为 `'100%'` 宽高，或基于 ScreenInfo 动态计算
  - [ ] 移除 `CANVAS_WIDTH` 固定常量（或保留仅作默认值）
  - [ ] 确保 `refreshOffCanvas()` / `clearTopCanvas()` 使用实际 canvas 尺寸
...
- **任务**：
  - [ ] 基于 `display.getDefaultDisplaySync()` 的实际宽高动态计算缩放比
  - [ ] 折叠屏展开态恢复全屏（当前已实现 `scaleValueX = 1`）
  - [ ] 折叠屏半折叠态（态3）适配：按实际显示区域比例缩放
  - [ ] 使用 `display.on('foldStatusChange')` + `display.getFoldDisplayStatus()` 获取折叠区域信息
...
### 5.1 断点系统引入
- **任务**：
  - [ ] 新增 `common/BreakpointSystem.ets`，定义断点枚举与计算逻辑
  - [ ] 断点定义：sm（<600vp 手机）、md（600~840vp 平板竖屏）、lg（>840vp 平板横屏/桌面）
  - [ ] 在 `aboutToAppear` 中计算当前断点，监听屏幕尺寸变化动态更新
  - [ ] 使用 `@StorageProp('currentBreakpoint')` 全局共享断点状态
...
- **现状**：底部固定 Row，6个按钮均分
- **任务**：
  - [ ] sm 断点：保持底部 Row 布局（当前样式）
  - [ ] md 断点：底部 Row 但按钮间距加大，图标/文字更大
  - [ ] lg 断点：改为左侧垂直 ToolBar + 右侧属性面板，画布占满剩余空间
  - [ ] 使用 `GridRow`/`GridCol` 栅格系统实现断点切换
...
- **任务**：
  - [ ] sm 断点：保持半模态弹窗
  - [ ] md 断点：半模态弹窗高度按屏幕比例计算
  - [ ] lg 断点：改为右侧常驻侧边面板，不遮挡画布
  - [ ] 面板内部布局也需响应式（颜色网格列数、滑块宽度等）
- **涉及文件**：`myPaintSheet.ets`、`Index.ets`
- **预估工作量**：1.5天

### 5.4 资源限定目录
- **任务**：
  - [ ] 新增 `resources/base/element/` 下的尺寸资源，按断点提供不同值
  - [ ] 图标资源：为高密度屏提供更高分辨率 SVG（当前 SVG 矢量已适配，主要补间距/字号）
  - [ ] 使用 `$r('app.float.xxx')` 替代硬编码数值，方便多设备覆盖
- **涉及文件**：`resources/` 目录
- **预估工作量**：1天
...
### 6.1 手写笔压感支持
- **任务**：
  - [ ] 监听 `TouchType.Down` 时判断 `event.touches[0].toolType` 是否为 `TouchToolType.Stylus`
  - [ ] 手写笔模式：读取 `event.touches[0].force`（压力值 0~1）动态调整线宽
  - [ ] 手写笔倾斜角：读取 `event.touches[0].tiltX/Y` 模拟钢笔笔锋方向
  - [ ] 手机触控模式保持当前逻辑不变
...
### 6.2 鼠标交互适配
- **任务**：
  - [ ] 鼠标右键：弹出上下文菜单（撤销/重做/清空/删除/复制）
  - [ ] 鼠标滚轮：缩放画布（替代双指缩放）
  - [ ] 鼠标中键拖拽：平移画布
  - [ ] 鼠标悬停：显示光标样式（画笔/橡皮/移动/十字）
  - [ ] 使用 `.mouseStyle()` 和 `onMouseEvent` 实现
...
### 7.1 平板多面板布局
- [ ] lg 断点实现三栏布局：左侧工具栏 + 中间画布 + 右侧属性面板
- [ ] 属性面板常驻显示：笔刷类型/颜色/粗细/不透明度/图层
- [ ] 面板可折叠/展开
- [ ] 画布区域支持全屏模式（隐藏面板）
...
- [ ] 支持从文件管理器拖拽图片到画布
- [ ] 支持从画布拖拽选中内容到其他应用
- [ ] 使用 `@kit.AbilityKit` 的 `DragBehavior` 实现

### 7.4 高刷新率适配
- [ ] 检测屏幕刷新率（`display.getDefaultDisplaySync().refreshRate`）
- [ ] 120Hz 屏幕下优化 Canvas 渲染频率
- [ ] 触摸采样率适配（高采样率下减少冗余点）

### 7.5 无障碍适配
- [ ] 为工具按钮添加 `accessibilityText`/`accessibilityDescription`
- [ ] 画布区域添加无障碍节点（描述画布内容）
- [ ] 支持屏幕阅读器朗读当前工具/颜色/粗细状态
- [ ] 高对比度模式下颜色方案适配
